...
if __name__ == '__main__':
    ...
    drive_model = tf.keras.models.Sequential([
        tf.keras.layers.LSTM(16, input_shape=(1,past_history), return_sequences=True),
        tf.keras.layers.Dense(8, activation = 'relu'),
        tf.keras.layers.Dense(4, activation = 'relu'),
        tf.keras.layers.Dense(1, activation='relu')
        ])
    drive_model.compile(optimizer='rmsprop', loss='mse', metrics=['mse'])
    ...
    stay_model.compile(optimizer='rmsprop', loss='mse', metrics=['mse'])
    
    # model train
    print('[+] Train model')
    drive_model.fit(train_d, epochs = EPOCH, steps_per_epoch=EVALUATION_INTERVAL, validation_data = train_d_val, validation_steps=50)
    stay_model.fit(train_s, epochs = EPOCH, steps_per_epoch=EVALUATION_INTERVAL, validation_data = train_s_val, validation_steps=50)
